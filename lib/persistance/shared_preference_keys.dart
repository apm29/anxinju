import 'package:shared_preferences/shared_preferences.dart';

SharedPreferences userSp;
const String KEY_TOKEN = "key_token";
const String KEY_REGISTRATION_ID = "KEY_REGISTRATION_ID";
const String KEY_DEVICE_TOKEN = "KEY_DEVICE_TOKEN";
const String KEY_FIRST_ENTRY_TAG = "KEY_FIRST_ENTRY_TAG";
const String KEY_USER_INFO = "key_user_info";
const String KEY_CURRENT_DISTRICT_INDEX = "key_current_district_index";
const String KEY_JSON_MENU_INDEX = "key_json_menu_index";
const String KEY_CURRENT_THEME_INDEX = "key_current_theme_index";
const String KEY_MESSAGE_SOUND = "KEY_MESSAGE_SOUND";
const String KEY_WEB_VIEW_CACHE_STAMP = "KEY_WEB_VIEW_CACHE_STAMP";